  ...
  },
  {
    ...
    kpis: [
      ...
      { id: "nflx-rev", label: "Revenue", unit: "$B", consensus: 12.2,
        explain: "Total sales from subscriptions and advertising combined. Netflix guided full-year 2026 revenue of $50.7-51.7 billion, which implies about $12.2B for Q1. Revenue growth has been steady at 15-18%.",
  ...
  },
  {
    ...
    kpis: [
      { id: "nvda-dc", label: "Data Center Revenue", unit: "$B", consensus: 72.0,
        explain: "Revenue from selling AI chips (GPUs) to Microsoft, Google, Meta, Amazon. This is 91% of NVIDIA's entire business. If this number disappoints even slightly, the stock tanks. THE number that tells you if the AI boom is still accelerating.",
        thresholds: [62, 66, 69, 72, 75, 79, 84] },
      { id: "nvda-gm", label: "Gross Margin", unit: "%", consensus: 75.0,
        explain: "How much profit NVIDIA keeps per chip after manufacturing costs. 75% is extraordinary. New chip architectures like Blackwell can temporarily compress this. Bears watch for signs of competition.",
...
function getExpectedValue(kpi) {
  ...
  for (let i = 1; i < prices.length; i++) ev += (prices[i].threshold - prices[i - 1].threshold) * (prices[i].price / 100);
  return ev;
}
...
function ProbCurve({ kpi }) {
  ...
  const prices = getThresholdPrices(kpi);
  const w = isMobile ? 260 : 280, h = 50, pad = 4;
        ...
        {prices.map((p, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={x + barW / 2} y={h + 12} textAnchor="middle" fill={isCon ? "#fff" : "#555"} fontSize={isMobile ? 7 : 8} fontFamily="monospace" fontWeight={isCon ? 700 : 400}>
                {p.threshold}{isPct ? "%" : ""}
              </text>
            </g>
          );
...
function BetInput({ value, onChange }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ textAlign: "center", fontSize: 28, fontWeight: 800, color: "#6ee7b7", fontFamily: "monospace" }}>${value}</div>
      <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "center" }}>
        <button onClick={() => onChange(Math.max(1, value - (value > 100 ? 50 : value > 25 ? 10 : 5)))} style={{ width: 36, height: 36, borderRadius: 7, border: "1px solid #f8717130", background: "#f8717108", color: "#f87171", fontSize: 18, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>-</button>
        <input type="number" value={value} onChange={e => onChange(Math.min(10000, Math.max(0, parseInt(e.target.value) || 0)))}
          style={{ width: 80, textAlign: "center", padding: "8px", background: "#0e0e20", border: "1px solid #ffffff15", borderRadius: 7, color: "#6ee7b7", fontSize: 16, fontWeight: 700, fontFamily: "monospace", outline: "none", MozAppearance: "textfield" }} />
        <button onClick={() => onChange(Math.min(10000, value + (value >= 100 ? 50 : value >= 25 ? 10 : 5)))} style={{ width: 36, height: 36, borderRadius: 7, border: "1px solid #6ee7b730", background: "#6ee7b708", color: "#6ee7b7", fontSize: 18, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
      </div>
      ...
      <div style={{ display: "flex", gap: 4, justifyContent: "center", flexWrap: "wrap" }}>
        {[10, 25, 50, 100, 250, 500].map(v => (
          <button key={v} onClick={() => onChange(v)} style={{ background: value === v ? "#6ee7b715" : "#ffffff05", border: "1px solid " + (value === v ? "#6ee7b7" : "#ffffff10"), color: value === v ? "#6ee7b7" : "#555", borderRadius: 4, padding: "4px 10px", fontSize: 10, cursor: "pointer", fontFamily: "monospace" }}>${v}</button>
        ))}
      </div>
    </div>
  );
...
function SignUpModal({ onClose, onSubmit }) {
  ...
  const ss = { width:"100%", padding:"10px 12px", background:"#0e0e20", border:"1px solid #ffffff15", borderRadius:8, color:"#e2e8f0", fontSize:14, fontFamily:"inherit", outline:"none", appearance:"none", WebkitAppearance:"none" };
  const ls = { fontSize:11, color:"#94a3b8", marginBottom:4, display:"block", fontFamily:"monospace" };
    ...
    <div style={{ position:"fixed", inset:0, zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(0,0,0,0.85)", backdropFilter:"blur(8px)", padding: 16 }}>
      <div style={{ width:"100%", maxWidth:440, maxHeight:"90vh", overflowY:"auto", background:"linear-gradient(180deg, #111128, #0a0a14)", border:"1px solid #ffffff12", borderRadius:16, padding: "24px 20px" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div><label style={ls}>Investment Expertise *</label><select value={form.expertise} onChange={set("expertise")} style={ss}><option value="">Select</option><option>Beginner</option><option>Casual</option><option>Active trader</option><option>Advanced (options)</option><option>Professional</option></select></div>
          <div><label style={ls}>Portfolio Size</label><select value={form.portfolioSize} onChange={set("portfolioSize")} style={ss}><option value="">Prefer not to say</option><option>Under $1K</option><option>$1K-$10K</option><option>$10K-$50K</option><option>$50K-$250K</option><option>$250K-$1M</option><option>$1M+</option></select></div>
          <div><label style={ls}>Trading Frequency</label><select value={form.tradingFreq} onChange={set("tradingFreq")} style={ss}><option value="">Select</option><option>Daily</option><option>Weekly</option><option>Monthly</option><option>Few times/year</option><option>Buy & hold</option></select></div>
          <div><label style={ls}>Venmo (for prizes)</label><input placeholder="@handle" value={form.venmo} onChange={set("venmo")} style={ss} /></div>
        ...
        <button disabled={!ok||loading} onClick={go} style={{ width:"100%", marginTop:18, padding:"14px 0", borderRadius:10, border:"none", background:ok?"linear-gradient(135deg, #059669, #6ee7b7)":"#333", color:ok?"#000":"#666", fontSize:14, fontWeight:800, cursor:ok?"pointer":"not-allowed", fontFamily:"monospace", letterSpacing:2, textTransform:"uppercase" }}>{loading?"Creating...":"Create Account"}</button>
      </div>
    </div>
  );
}

function KPIMarketCard({ kpi, co, onBuy, isMobile }) {
  ...
  return (
    <div style={{ background: "#0c0c18", border: "1px solid #ffffff0a", borderRadius: 14, overflow: "hidden" }}>
      <div style={{ padding: isMobile ? "12px 14px 10px" : "14px 16px 10px", cursor: "pointer" }} onClick={() => setExpanded(!expanded)}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6, gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontSize: isMobile ? 14 : 15, fontWeight: 700, color: "#e2e8f0" }}>{kpi.label}</span>
            <button onClick={e => { e.stopPropagation(); setShowExplain(!showExplain); }} style={{ background: "#ffffff08", border: "1px solid #ffffff12", borderRadius: 4, color: "#6b7280", fontSize: 9, padding: "2px 6px", cursor: "pointer", fontFamily: "monospace" }}>?</button>
          </div>
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            <div style={{ fontSize: 8, color: "#6b7280", fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1 }}>Mkt Expects</div>
            <div style={{ fontSize: isMobile ? 15 : 17, fontWeight: 800, color: "#6ee7b7", fontFamily: "monospace" }}>{ev.toFixed(isPct ? 1 : kpi.consensus < 10 ? 2 : 1)}<span style={{ fontSize: 9, color: "#555", marginLeft: 2 }}>{kpi.unit}</span></div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: isMobile ? 10 : 11, color: "#6b7280", fontFamily: "monospace" }}>Consensus: <span style={{ color: "#fff", fontWeight: 600 }}>{kpi.consensus}{isPct ? "%" : ""}</span></div>
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid #ffffff08" }}>
          {/* Header */}
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 40px 40px" : "1fr 55px 55px 55px 55px", padding: "6px 12px", gap: isMobile ? 4 : 6, fontSize: 9, color: "#555", fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1 }}>
            <div>Threshold</div>
            {isMobile ? <><div style={{ textAlign:"center" }}>Yes</div><div style={{ textAlign:"center" }}>No</div></> :
              <><div style={{ textAlign:"center" }}>Yes</div><div style={{ textAlign:"center" }}>No</div><div></div><div></div></>}
          </div>
          {prices.map(p => (
            <div key={p.threshold} style={{
              display: "grid",
              gridTemplateColumns: isMobile ? "1fr 40px 40px" : "1fr 55px 55px 55px 55px",
              alignItems: "center", padding: isMobile ? "6px 12px" : "7px 12px", gap: isMobile ? 4 : 6,
              borderBottom: "1px solid #ffffff06", fontSize: 12,
            }}>
              <div style={{ fontFamily: "monospace", fontWeight: p.threshold === kpi.consensus ? 700 : 500, color: p.threshold === kpi.consensus ? "#fff" : "#94a3b8", fontSize: isMobile ? 11 : 12 }}>
                &gt; {p.threshold}{isPct ? "%" : ""}
                {p.threshold === kpi.consensus && <span style={{ fontSize: 7, color: "#6ee7b7", marginLeft: 4, letterSpacing: 1 }}>CON</span>}
              </div>
              {isMobile ? (
                <>
                  <button onClick={() => onBuy("yes", p.threshold, p.price)} style={{ padding: "6px 0", borderRadius: 5, border: "1px solid #6ee7b730", background: "#6ee7b70a", color: "#6ee7b7", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "monospace" }}>{p.price}c</button>
                  <button onClick={() => onBuy("no", p.threshold, 100 - p.price)} style={{ padding: "6px 0", borderRadius: 5, border: "1px solid #f8717130", background: "#f871710a", color: "#f87171", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "monospace" }}>{100-p.price}c</button>
                </>
              ) : (
                <>
...
function TradeSlip({ co, kpi, side, threshold, price, onClose, user, onRequireLogin, isMobile }) {
  ...
  const go = async () => {
    ...
    await sendEmail("[BetKPI] " + user.username + " | " + co.ticker + " " + kpi.label + " " + side.toUpperCase() + " >" + threshold, {
      message: "BET\n\nUser: " + user.username + " (" + user.email + ")\nExpertise: " + user.expertise + "\nPortfolio: " + (user.portfolioSize||"N/A") + "\n\n" + co.ticker + " " + kpi.label + "\n>" + threshold + (isPct?"%":" "+kpi.unit) + " " + side.toUpperCase() + "\nPrice: " + price + "c | Wager: $" + wager + " | Contracts: " + contracts,
      from_name: "BetKPI",
    });
  ...
  const wrapStyle = isMobile ? {
    ...
    background: "linear-gradient(180deg, #111128, #0a0a12)",
    border: "none", borderTop: "1px solid #ffffff15", borderRadius: "16px 16px 0 0",
    padding: "20px 18px", paddingBottom: 28,
    maxHeight: "80vh", overflowY: "auto",
    boxShadow: "0 -10px 40px rgba(0,0,0,0.8)",
  } : {
    background: "linear-gradient(180deg, #111128, #0a0a12)",
    border: "1px solid #ffffff10", borderRadius: 14, padding: 20, position: "relative",
  };
    ...
    <div style={wrapStyle}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 6, justifyContent: "center", marginBottom: 14 }}>
          <a href={"https://twitter.com/intent/tweet?text=" + encodeURIComponent(shareText)} target="_blank" rel="noreferrer" style={{ padding: "10px 16px", borderRadius: 7, background: "#1d9bf010", border: "1px solid #1d9bf040", color: "#1d9bf0", fontSize: 12, fontWeight: 600, textDecoration: "none", fontFamily: "monospace" }}>Share on X</a>
          <button onClick={() => navigator.clipboard.writeText(shareText)} style={{ padding: "10px 16px", borderRadius: 7, background: "#ffffff08", border: "1px solid #ffffff15", color: "#94a3b8", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "monospace" }}>Copy</button>
        </div>
        <button onClick={() => { setSubmitted(false); onClose(); }} style={{ padding: "10px 24px", borderRadius: 7, border: "1px solid #ffffff15", background: "#ffffff08", color: "#e2e8f0", fontSize: 12, cursor: "pointer", fontFamily: "monospace" }}>Another Bet</button>
      </div>
    </div>
  );
  ...
  return (
    <>
      {isMobile && <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 80, background: "rgba(0,0,0,0.6)" }} />}
      <div style={wrapStyle}>
        <button onClick={onClose} style={{ position: "absolute", top: 12, right: 14, background: "none", border: "none", color: "#555", cursor: "pointer", fontSize: 20 }}>x</button>
        ...
        <div style={{ marginTop: 14, padding: 12, background: "#ffffff04", borderRadius: 8, border: "1px solid #ffffff06" }}>
          {[["Price", price + "c"], ["Contracts", contracts], ["Potential Payout", "$" + contracts + ".00"]].map(([k, v], i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", marginBottom: i < 2 ? 5 : 0 }}>
              ...
              <span style={{ fontSize: 11, color: i === 2 ? "#6ee7b7" : "#ddd", fontWeight: i === 2 ? 700 : 400, fontFamily: "monospace" }}>{v}</span>
            </div>
          ))}
        </div>
        <button disabled={loading} onClick={go} style={{
          width: "100%", marginTop: 12, padding: "14px 0", borderRadius: 8, border: "none",
          ...
          color: "#000", fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "monospace", letterSpacing: 2, textTransform: "uppercase",
...
function HeroExample() {
  ...
  return (
    <div style={{ background: "#0c0c18", border: "1px solid #ffffff0a", borderRadius: 14, padding: "14px 16px", width: "100%", maxWidth: 340 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
...
export default function BetKPI() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#08080f", color: "#e2e8f0", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      {/* NAV */}
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "10px 14px" : "11px 22px", borderBottom: "1px solid #ffffff08", position: "sticky", top: 0, zIndex: 50, background: "#08080fee", backdropFilter: "blur(16px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 26, height: 26, borderRadius: 6, background: "linear-gradient(135deg, #6ee7b7, #059669)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 900, color: "#000", fontFamily: "monospace" }}>B</div>
          <span style={{ fontSize: 15, fontWeight: 800, fontFamily: "'Sora', sans-serif", letterSpacing: -0.5 }}>bet<span style={{ color: "#6ee7b7" }}>kpi</span></span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#6ee7b7", animation: "blink 2s infinite" }} />
          {!isMobile && <span style={{ fontSize: 9, color: "#6b7280", fontFamily: "monospace", letterSpacing: 1 }}>LIVE</span>}
          {user ? (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: 6 }}>
              <div style={{ padding: "5px 10px", borderRadius: 6, background: "#6ee7b715", border: "1px solid #6ee7b740", color: "#6ee7b7", fontSize: 11, fontWeight: 600, fontFamily: "monospace" }}>{user.username}</div>
              <button onClick={() => { localStorage.removeItem("betkpi_user"); setUser(null); }} style={{ background: "none", border: "none", color: "#555", cursor: "pointer", fontSize: 10, fontFamily: "monospace" }}>x</button>
            </div>
          ) : (
            <button onClick={() => setShowSignUp(true)} style={{ marginLeft: 6, padding: "5px 12px", borderRadius: 6, border: "1px solid #6ee7b750", background: "#6ee7b710", color: "#6ee7b7", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Sign Up</button>
          )}
        </div>
      </nav>

      <div style={{ maxWidth: 1060, margin: "0 auto", padding: isMobile ? "0 14px" : "0 18px" }}>
        {/* HERO */}
        <div style={{ padding: isMobile ? "28px 0 20px" : "40px 0 32px", animation: "fadeUp 0.5s ease" }}>
          <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 20 : 36, alignItems: isMobile ? "stretch" : "center" }}>
            <div style={{ flex: "1 1 320px" }}>
              ...
              <h1 style={{ fontSize: isMobile ? 28 : 34, fontWeight: 800, fontFamily: "'Sora', sans-serif", letterSpacing: -1.5, lineHeight: 1.1, marginBottom: 10, color: "#fff" }}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 18, padding: "8px 0", borderBottom: "1px solid #ffffff08", overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
          {["ALL", ...COMPANIES.map(c => c.ticker)].map(t => {
            ...
            return (<button key={t} onClick={() => setActiveTab(t)} style={{ padding: isMobile ? "7px 12px" : "6px 14px", borderRadius: 7, border: "1px solid " + (isA ? col + "50" : "#ffffff0d"), background: isA ? col + "10" : "transparent", color: isA ? col : "#6b7280", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "monospace", letterSpacing: 1, whiteSpace: "nowrap" }}>{t}</button>);
          })}
        </div>

        {/* MAIN */}
        <div style={{ display: isMobile ? "block" : "grid", gridTemplateColumns: trade && !isMobile ? "1fr 310px" : "1fr", gap: 22 }}>
          <div>
            {companies.map(co => (
              <div key={co.ticker} style={{ marginBottom: 28 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10, padding: isMobile ? "10px 12px" : "12px 16px", background: co.color + "06", border: "1px solid " + co.color + "15", borderRadius: 11 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 30, height: 30, borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", background: co.color + "18", border: "1px solid " + co.color + "35", fontSize: 9, fontWeight: 800, color: co.accent, fontFamily: "monospace" }}>{co.ticker.slice(0,2)}</div>
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ fontSize: 14, fontWeight: 700 }}>{co.ticker}</span><span style={{ fontSize: 9, color: "#555", fontFamily: "monospace" }}>{co.mcap}</span></div>
                      <div style={{ fontSize: 10, color: "#6b7280" }}>{co.name}</div>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 7, color: "#6b7280", textTransform: "uppercase", letterSpacing: 1.5, fontFamily: "monospace", marginBottom: 2 }}>Earnings In</div>
                    <Countdown targetDate={co.nextEarnings} />
                  </div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {co.kpis.map(kpi => (
                    <KPIMarketCard key={kpi.id} kpi={kpi} co={co} isMobile={isMobile} onBuy={(side, threshold, price) => handleBuy(co, kpi, side, threshold, price)} />
                  ))}
                </div>
              </div>
            ))}
              ...
              {LEADERS.map((l,i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 0", borderBottom: i < LEADERS.length-1 ? "1px solid #ffffff06" : "none" }}>
                  ...
                  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    ...
                    <span style={{ fontSize: 11, color: "#6ee7b7", fontWeight: 700, fontFamily: "monospace" }}>{l.pnl}</span>
                  </div>
                </div>
              ))}
          ...
          {trade && !isMobile && (
            <div style={{ position: "sticky", top: 68, alignSelf: "start" }}>
              <TradeSlip co={trade.co} kpi={trade.kpi} side={trade.side} threshold={trade.threshold} price={trade.price} onClose={() => setTrade(null)} user={user} onRequireLogin={() => setShowSignUp(true)} isMobile={false} />
            </div>
          )}
        </div>
        ...
        {/* Mobile bottom sheet */}
        {trade && isMobile && (
          <TradeSlip co={trade.co} kpi={trade.kpi} side={trade.side} threshold={trade.threshold} price={trade.price} onClose={() => setTrade(null)} user={user} onRequireLogin={() => setShowSignUp(true)} isMobile={true} />
        )}

        {/* HOW IT WORKS */}
        <div style={{ margin: "24px 0", padding: isMobile ? "18px 16px" : "22px 24px", background: "#0c0c18", border: "1px solid #ffffff08", borderRadius: 13 }}>
        ...
        {/* WHY */}
        <div style={{ margin: "0 0 32px", padding: isMobile ? "18px 16px" : "22px 24px", background: "#0c0c18", border: "1px solid #ffffff08", borderRadius: 13 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: isMobile ? 14 : 20 }}>
            {[
              { t:"Purer than stocks", d:"A stock moves on 50 things. A KPI market moves on one number." },
              { t:"See crowd expectations", d:"7 thresholds show the probability curve. Spot where the market is wrong." },
              { t:"Asymmetric upside", d:"A 15c contract pays 6.7x if right. Find mispriced thresholds." },
            ].map((x,i) => (
              <div key={i}><div style={{ fontSize: 13, fontWeight: 700, color: "#e2e8f0", marginBottom: 3 }}>{x.t}</div><div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.5 }}>{x.d}</div></div>
            ))}
          </div>
        </div>

        <div style={{ padding: "18px 0", borderTop: "1px solid #ffffff06", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
